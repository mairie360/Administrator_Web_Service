...
const [{ middleware }, { buildContentSecurityPolicy }, { default: nextConfig }] = loadTs([
  'src/middleware.ts',
...
const pageRequest = (token) => new NextRequest('http://localhost:5000/', token ? { headers: { cookie: `accessToken=${token}` } } : {});
