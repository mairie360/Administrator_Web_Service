...
// Lecture du contrat OpenAPI 3.1 de BFF User et validation JSON Schema minimale (même validateur que
// les tests des BFFs, BFF_user/tests/support/openapi-contract.ts). Il ne couvre que les mots-clés émis
// par zod-to-openapi : tout mot-clé inconnu fait échouer la validation plutôt que d'être ignoré.
...
class OpenApiContract {
  ...
  /** Toutes les opérations déclarées, sous la forme { method: 'GET', template }. */
  operations() {
    return Object.entries(this.document.paths).flatMap(([template, item]) => Object.keys(item)
      ...
      .map((method) => ({ method: method.toUpperCase(), template })));
  }
  ...
  /** Vérifie qu'une requête reçue par le BFF correspond à une opération déclarée. */
